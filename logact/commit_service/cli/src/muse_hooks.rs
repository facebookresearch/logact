...
/// Arguments for a Muse hook invocation.
#[derive(Args, Debug)]
pub struct HookArgs {
    #[command(subcommand)]
    event: HookEvent,
...
enum HookEvent {
    ...
    UserPromptSubmit,
    /// Muse is about to execute a tool: gate as an Intention.
    PreToolUse,
    /// A tool just finished successfully: record as an ActionOutput.
    PostToolUse,
    /// Muse finished a turn: record as an AgentOutput.
    Stop,
}
...
/// Translate a rejected AgentBus decision into Muse's PreToolUse response.
/// Approval is empty stdout so Muse's native permission flow remains intact.
