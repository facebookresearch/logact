...
//!
//! This service connects to a remote AgentBus via gRPC and continuously polls for
...
async fn main(_fb: FacebookInit) -> Result<()> {
    ...
    let _environment = Rc::new(RealEnvironment::new());
    ...
    let shutdown_fut = shutdown_signal.fuse();
    let run_fut = decider.run(None).fuse();
    futures::pin_mut!(shutdown_fut, run_fut);
    ...
    let result = futures::select_biased! {
        ...
        result = run_fut => result.map_err(anyhow::Error::from),
    };
...
async fn wait_for_agentbus_connection(
    ...
    let result = Retry::start(retry_strategy, || async {
        ...
        agent_bus_client
            .check_tail(request)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to reach AgentBus: {:?}", e))
    })
    .await;
    ...
    match result {
        Ok(resp) => {
            info!(
                tail = resp.tail_position,
                "Successfully connected to AgentBus"
            );
            Ok(())
        }
        Err(e) => {
