...
use agentbus_api::environment::MonotonicInstant;
use agentbus_api::environment::UnsafeWallTime;
...
/// Returns `AGENTBUS_SIMULATOR_SEED`, or a random seed when it is unset.
pub fn generate_seed() -> u64 {
    match std::env::var("AGENTBUS_SIMULATOR_SEED") {
        Ok(value) => value
            .parse()
            .unwrap_or_else(|error| panic!("invalid AGENTBUS_SIMULATOR_SEED '{value}': {error}")),
        Err(std::env::VarError::NotPresent) => rand::random(),
        Err(std::env::VarError::NotUnicode(_)) => {
            panic!("AGENTBUS_SIMULATOR_SEED is not valid Unicode")
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct RunStats {
    /// Number of event-loop steps (each step polls one task)
    pub polls: usize,
    /// Total tasks spawned (events + sleeps)
...
impl Simulator {
    ...
    pub fn run(&self) -> RunStats {
        let seed = self.rng.borrow_mut().random();
...
impl DeterministicExecutor {
    pub fn new() -> Self {
        let uniform = Uniform::new(100, 501).unwrap();
        Self::with_jitter(uniform)
    }
        ...
        F: Future<Output = ()> + 'static,
    {
        let sequence = *self.next_sequence.borrow();
        *self.next_sequence.borrow_mut() += 1;
        let target_time = clock.elapsed_time() + duration;
        let task = Task {
    ...
    fn next_event_time(
        ...
        Some(clock.elapsed_time() + std::time::Duration::from_micros(jitter_us))
    }

    fn next_sleep_time(&self) -> Option<std::time::Duration> {
        self.sleeps
            .borrow()
            .peek()
    ...
    pub fn run(&self, rng: &mut StdRng, clock: &LogicalClock) -> RunStats {
        ...
        loop {
            ...
            let (task_to_run, target_time) = match (next_event_time, next_sleep_time) {
                ...
                (Some(event_time), _) => {
                    let n = self.events.borrow().len();
                    let event_index = rng.random_range(0..n);
        ...
        RunStats {
            polls: iteration_count,
            total_events: *self.next_sequence.borrow() as usize,
...
impl LogicalClock {
    ...
    pub fn advance_to(&self, target_time: std::time::Duration) {
        let mut current = self.elapsed.borrow_mut();
...
impl Clock for LogicalClock {
    fn unsafe_wall_time(&self) -> UnsafeWallTime {
        UnsafeWallTime::from_duration_since_epoch(self.elapsed_time())
    }

    fn monotonic_time(&self) -> MonotonicInstant {
        MonotonicInstant::from_duration_since_clock_origin(self.elapsed_time())
    }
}

/// Deterministic barrier for use with the simulator.
///
/// Unlike `tokio::sync::Barrier`, this implementation guarantees deterministic
/// wake ordering: waiters are woken in the order they arrived at the barrier.
pub struct SimulatorBarrier {
    n: usize,
    state: Rc<RefCell<SimulatorBarrierState>>,
}

struct SimulatorBarrierState {
    arrived: usize,
...
impl SimulatorBarrier {
    pub fn new(n: usize) -> Rc<Self> {
        Rc::new(Self {
            n,
            state: Rc::new(RefCell::new(SimulatorBarrierState {
                arrived: 0,
                senders: Vec::new(),
            })),
        })
    ...
    pub async fn wait(&self) {
        let receiver = {
            ...
            if state.arrived == self.n {
                // All arrived — wake everyone in arrival order (deterministic FIFO)
