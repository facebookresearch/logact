...
fn test_environment_trait_sleep() {
    fn test_with_env<E: Environment>(
        env: &mut E,
        duration: Duration,
    ) -> impl std::future::Future<Output = MonotonicInstant> + 'static + use<'_, E> {
        let start_time = env.with_clock(|clock| clock.monotonic_time());
    ...
    let mut env = Simulator::with_jitter(seed, Uniform::new(1, 101).unwrap());
    ...
    let end_time = env.with_clock(|clock| clock.monotonic_time());
...
fn test_multiple_simultaneous_sleeps() {
    fn run_scenario(seed: u64) -> (Duration, Vec<usize>) {
        let env = Simulator::with_jitter(seed, Uniform::new(100, 200).unwrap());
        let execution_order = Rc::new(RefCell::new(Vec::new()));

        let ms = env.rng.borrow_mut().random_range(1..=100);
        let sleep_duration = Duration::from_millis(ms);
        let start_time = env.clock.monotonic_time();
        ...
        let total_elapsed = env.clock.monotonic_time() - start_time;
...
fn test_staggered_sleeps() {
    fn run_scenario(seed: u64) -> (Duration, Duration, Vec<usize>) {
        let env = Simulator::with_jitter(seed, Uniform::new(1, 101).unwrap());
        let start_time = env.clock.monotonic_time();
        let execution_order = Rc::new(RefCell::new(Vec::new()));
...
fn test_simulator_determinism() {
    fn run_scenario(seed: u64) -> (Vec<i32>, Vec<i32>) {
        let env = Simulator::with_jitter(seed, Uniform::new(0, 501).unwrap());
        let shared_results = Rc::new(RefCell::new(Vec::new()));
        let mut handles = Vec::new();

        let num_tasks = env.with_rng(|rng| rng.random_range(1..=10));

        // Spawn tasks that use RNG
        for _ in 0..num_tasks {
            let seed: u64 = env.with_rng(|rng| rng.random());
            let results_clone = shared_results.clone();
            let handle = env.spawn(async move {
                let mut task_rng = StdRng::seed_from_u64(seed);
                let value = task_rng.random_range(1..=100);
...
fn test_many_senders_one_receiver() {
    ...
    let stats = env.run();
    assert_eq!(stats.polls, 1, "expected single iteration");
...
fn test_small_jitter_large_sleep() {
    let seed: u64 = rand::random();
    let env = Rc::new(Simulator::with_jitter(seed, Uniform::new(1, 2).unwrap()));
    let start_time = env.clock.monotonic_time();
    ...
    let elapsed = env.clock.monotonic_time() - start_time;
    assert!(
        elapsed >= sleep_duration,
...
fn test_zero_jitter_with_sleep() {
    let seed: u64 = rand::random();
    let env = Rc::new(Simulator::with_jitter(seed, Uniform::new(0, 1).unwrap()));
    let start_time = env.clock.monotonic_time();
...
fn test_minimal_iterations() {
    ...
    for _ in 0..100 {
        ...
        assert_eq!(stats.polls, 9); // 2 polls per rx and 3 for the sender and its sleep.
    }
}
