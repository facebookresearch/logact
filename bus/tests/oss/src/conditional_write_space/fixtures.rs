...
use agentbus_dynamodb::DynamoConditionalWriteSpace;
...
const TABLE_NAME: &str = "agentbus_cws_test";
const SPACE_ID_COLUMN: &str = "space_id";
const ADDRESS_COLUMN: &str = "address";
const DYNAMODB_PORT: u16 = 8000;

pub struct DynamoConditionalWriteSpaceFixture {
    space: DynamoConditionalWriteSpace,
    env: Rc<RealEnvironment>,
    _container: ContainerAsync<GenericImage>,
}

impl ConformanceFixture for DynamoConditionalWriteSpaceFixture {
    type Env = RealEnvironment;
    type Impl = DynamoConditionalWriteSpace;
...
impl DynamoConditionalWriteSpaceFixture {
    pub async fn new_async(_fb: FacebookInit) -> Result<Self> {
        ...
        let space = DynamoConditionalWriteSpace::new(client, TABLE_NAME.to_string()).await?;
        Ok(Self {
            space,
            env,
...
impl IntegrationFixture for DynamoConditionalWriteSpaceFixture {
